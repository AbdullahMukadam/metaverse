...
function SignInComponent() {
    ...
    const githubSignIn = useCallback(async () => {
        try {
            ...
            toast("An error Occured in SignIn")
            console.log(error)
            setloading(false)
        }
    }, [],)
    ...
    return (
        <div className='w-full h-[80%] flex items-center justify-center flex-col font-michroma p-2 bg-gradient-to-br from-[#C4C4C4] via-slate-200 to-[#F2F2F2] rounded-2xl border-2 border-black'>
            <h1 className='font-bold text-center'>SignIn</h1>
            <p className='text-sm text-center mt-2'>Login to your Account</p>
            <div className='w-full flex items-center justify-center flex-col mt-2 gap-2'>
                <button onClick={githubSignIn} disabled={loading} className='p-2 rounded-lg border-2 border-black flex w-[250px] items-center justify-center gap-3 cursor-pointer transition-all duration-300 ease-in-out hover:bg-zinc-950 hover:text-white'>
                    {loading ? <Spinner /> : (
                ...
                <button onClick={googleSignIn} disabled={loading} className='p-2 rounded-lg border-2 border-black flex w-[250px] items-center justify-center gap-3 cursor-pointer transition-all duration-300 ease-in-out hover:bg-zinc-950 hover:text-white'>
                    {loading ? <Spinner /> : (
