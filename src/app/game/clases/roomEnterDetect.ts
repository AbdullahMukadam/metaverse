...
export class roomEnterDetect {
    public roomEnterDetectArray: number[][] = []
    ...
    constructor(roomEnterDetectArray: number[][]) {
        this.roomEnterDetectArray = roomEnterDetectArray
        this.createBoundaryArray()
    }

    createBoundaryArray() {
        this.roomEnterDetectArray.forEach((row, i) => {
            row.forEach((tile, j) => {
                if (tile === 1025) {
    ...
    detectRoomEnterZone(character: Character): boolean {
